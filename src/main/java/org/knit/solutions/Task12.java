...
import org.knit.solutions.task12.*;
...
@TaskDescription(taskNumber = 12, taskDescription = "Задача «Реализация системы отмены изменений в текстовом редакторе»")
...
public class Task12 implements Solution{
    ...
    public void execute() {
        ...
        textEditor.addText(" How are you?");
        textEditor.saveState();

        textEditor.addText(" Have a good day!");
        textEditor.saveState();

        System.out.println("Текст: " + textEditor.getCurrentText());

        textEditor.undo();
        System.out.println("После 1 отката: " + textEditor.getCurrentText());

        textEditor.undo();
        System.out.println("После 2 отката: " + textEditor.getCurrentText());

        textEditor.undo();
        System.out.println("После 3 отката: " + textEditor.getCurrentText());
    }
}
